...
function firework(time) {
    ...
    import('https://unpkg.com/fireworks-js@2.x/dist/index.umd.js')
        ...
        Object.keys(customStyle).forEach((v) => fireworkDiv.style[v] = customStyle[v]);
        // @ts-expect-error
...
function addCustomOrderProperty(arrayOfObject) {
    return arrayOfObject.map((v, i) => { v.customOrderProperty = i; return v; });
}
function drawOrganized(SLICE_NUMBER_IN_ROOT, organizedSelector = '#organized') {
    ...
    const organized = Array(Math.pow(SLICE_NUMBER_IN_ROOT, 2)).fill(0)
        .map(v => {
    ...
    function createDivElement(width) {
...
function assignToGLOBAL_ELEMENT(element) {
    GLOBAL_ELEMENT = element;
}
function getGLOBAL_ELEMENT() {
    return GLOBAL_ELEMENT;
}
function assignToGLOBAL_NEW_GAME(isTrue) {
    GLOBAL_NEW_GAME = isTrue;
}
function checkResult() {
    const validity = [...document.querySelector('#organized').children].every((v, i) => v.customOrderProperty == i);
