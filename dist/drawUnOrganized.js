...
function drawDisOrganized(file, SLICE_NUMBER_IN_ROOT) {
    document.querySelectorAll('.grid')
        .forEach(v => v.style.gridTemplateColumns = `repeat(${SLICE_NUMBER_IN_ROOT},auto)`);
    const width = (document.querySelector(organizedSelector).clientWidth - 10) / SLICE_NUMBER_IN_ROOT;
    return getSlicedImages(file, SLICE_NUMBER_IN_ROOT)
        ...
        //        .then(v => v.map(v => {v.style.width = '100%'; return v;}))
